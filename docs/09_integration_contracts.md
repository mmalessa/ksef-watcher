...
|---|---|---|---|---|
| KSeF API 2.0 | Ministry of Finance (KSeF) | `KsefWatcher.KsefAccess` | Whatever the official `CIRFMF/ksef-client-csharp` NuGet package models (`IInvoiceDownloadClient.QueryInvoiceMetadataAsync`, response shapes) | **Pin an exact package version** in `KsefWatcher.KsefAccess.csproj`; bump deliberately, re-verify against the sandbox environments (`api-test`/`api-demo`, A13) before shipping a bump. KSeF Access's own ACL translation (`InvoiceSummary → DetectedInvoice`, `PollFailure` classification — `08_ksef_access_tactical_model.md`) is the only "schema" this project maintains itself. |
