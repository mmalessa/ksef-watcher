...
flowchart TD
    ...
    SCH --> REM["removed subject → stops polling +<br/>state reset (I-19, OQ-15: baseline afresh on re-add)"]
    SCH --> CHG["changed interval → recompute poll offset (A9),<br/>next timer fires per new interval + offset"]
```

**Narration.** The daemon watches the config file; on change it validates and, if valid, republishes the configuration — no restart. Validation includes the **per-subject minimum interval (I-13a: ≥ 15 min, MF recommendation)**: a file with any subject below the bound is *invalid* — rejected here and refused at startup. There is **no global fleet cap** — KSeF counts limits per (context + IP) pair, so each subject brings its own budget (I-21). A **newly added subject** has no state (`lastHwm = null`, empty registry), so its first poll establishes the **baseline** (I-18): one narrow fetch, no notifications sent, `lastHwm` set — only invoices arriving after onboarding are notified. A **removed subject** stops being polled **and its state is deliberately reset** (I-19, OQ-15: baseline afresh on re-add — removal is a conscious relinquish of the absence period's notifications, the only state-changing consequence of a reload per I-20). A changed interval takes effect from the next poll, with the poll offset recomputed (A9 — offset depends on the interval). Invalid file ⇒ keep last valid config, log loudly (I-16; fail-fast I-13 applies only at startup).

## Open questions (new)

- **OQ-7a/7b** *Resolved (jointly): watchdog heartbeat per subject.* Once a day (fixed time per subject, derived from its poll offset for spreading), the subject's channel receives a short *"no new invoices (as of {date})"* heartbeat — sent through the normal delivery path (the cycle's `INotifier`), so the pulse exercises the whole pipeline: daemon liveness + channel health. The **absence of an expected heartbeat is the alarm** (inverted liveness check — a dead system cannot announce its own death, but a missing pulse can be noticed; self-checking, unlike a failure alert that could itself die silently). OQ-7b is thereby resolved *for V1*: the heartbeat **replaces** a dedicated operator fallback channel — a permanently broken channel is first noticed by its heartbeat going missing, backed by loud permanent-failure logs (I-11). Mechanics note: the heartbeat message says nothing new (it is a pulse, not a report) — a subject that *did* receive invoices that day simply still gets the heartbeat at its fixed time; the daily pulse time is per-subject, derived from the poll offset (A9-style spreading), decided here as domain policy, scheduled by Step 9.